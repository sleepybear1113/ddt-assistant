...
@Data
@Slf4j
public class Pc2PredictDto extends BasePredictDto implements Serializable {
    ...
    public Pc2PredictDto() {
    }

    public Pc2PredictDto(String imgFile) {
        setImgFile(imgFile);
    }
    ...
    public String getHost(boolean throwException) {
        String host = SpringContextUtil.getBean(CaptchaConfig.class).getPc2().getServerAddr();
    ...
    public void build(CaptchaConfig captchaConfig, String imgFilePath) {
        ...
        Pc2Captcha pc2 = captchaConfig.getPc2();
        if (!pc2.validUserInfo()) {
            throw new FrontException("用户卡密信息错误！");
        }
        getHost(true);
        setAuthor(pc2.getAuthor());
        setCami(pc2.getCami());
    }
    ...
    public String getAccountInfo(CaptchaConfig captchaConfig) {
        String cami = captchaConfig.getPc2().getCami();
        String author = captchaConfig.getPc2().getAuthor();
        Boolean lowBalanceRemind = captchaConfig.getLowBalanceRemind();
        Double lowBalanceNum = captchaConfig.getLowBalanceNum();

        StringBuilder sb = new StringBuilder(CaptchaChoiceEnum.PC2.getName());
        ...
        try {
            ...
            PcAccountInfo pc2AccountInfo = Util.parseJsonToObject(responseBody, PcAccountInfo.class, mapper);
            if (pc2AccountInfo == null) {
                return sb.append("解析平台返回内容失败").toString();
            }

            // 低余额提醒
            String balance = pc2AccountInfo.getBalance();
            if (Util.isNumber(balance) && Boolean.TRUE.equals(lowBalanceRemind) && lowBalanceNum != null && lowBalanceNum > 0) {
                ...
                if (realtimeBalance / 100 < lowBalanceNum) {
                    log.warn(CaptchaChoiceEnum.PC2.getName() + "当前余额：{}，低于设定值 {}，请注意！", realtimeBalance, lowBalanceNum);
            ...
            return String.format(CaptchaChoiceEnum.PC2.getName() + " 当前余额：%s", balance);
        } catch (Exception e) {
    ...
    public static void main(String[] args) {
        Pc2PredictDto pc2PredictDto = new Pc2PredictDto("test/A@66142-12_54_44.png");
        CaptchaConfig captchaConfig = new CaptchaConfig();
        Pc2Captcha pc2 = new Pc2Captcha();
        pc2.setAuthor("sleepy");
        pc2.setCami("YGc6Jb0W5jzu72e0");
        captchaConfig.setPc2(pc2);

        pc2PredictDto.build(captchaConfig, "test/A@66142-12_54_44.png");
        pc2PredictDto.getAccountInfo(captchaConfig);
    }
}
