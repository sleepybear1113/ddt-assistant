...
public class HttpHelper implements Serializable {
    ...
    public HttpHelper(HttpRequestMaker httpRequestMaker) {
        ...
        String userAgent = VersionInfo.getUserAgent("Apache-HttpClient", "org.apache.http.client", this.getClass()) + "-" + GlobalVariable.version;
        this.httpClient = HttpClientBuilder.create().setSSLSocketFactory(sslConnectionSocketFactory).setDefaultRequestConfig(RequestConfig.custom().setCookieSpec(CookieSpecs.STANDARD).build()).setDefaultCookieStore(httpCookieStore).setUserAgent(userAgent).build();
    }
