package cn.sleepybear.ddtassistant.type.auction;
...
import cn.sleepybear.ddtassistant.type.common.Common;
...
public class Auction extends BaseType {
    ...
    public static boolean startSellThread(int hwnd, boolean confirm, BindWindowConfig bindWindowConfig) {
        if (isRunning(hwnd, Auction.class)) {
            return false;
        }
        GlobalVariable.THREAD_POOL.execute(() -> Auction.createInstance(hwnd, Auction.class, false, bindWindowConfig).goLoop(confirm));
        return true;
    }
